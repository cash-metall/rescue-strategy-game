import type { UnitType, Terrain, WeatherKey, BuildKey, Color, ProfileBase, IncidentKey, Outcome } from './types';
 ...
 * Поисковикам никто не платит: отряд живёт на пожертвования, а жертвуют тогда, когда есть что
 * показать. Поэтому деньги — функция ПРОДЕЛАННОЙ РАБОТЫ, а не прошедшего времени.
 *
 * Отклик копится за прочёсанные квадраты и разобранные находки; на каждом пороге приходит транш
 * пожертвований. Три правила, без которых модель ломается:
 *  1. Отклик даётся только за НОВОЕ покрытие карты (прирост `cell.coverage`), иначе две группы
 *     фармят его, гоняя туда-сюда по одному квадрату.
 *  2. Разобранная находка приносит отклик независимо от того, улика это или мусор: иначе скачок
 *     счётчика выдавал бы игроку правду и убивал сверку с приметами.
 *  3. Интерес к делу затухает со временем — иначе затянуть поиск снова становится выгодно.
 */
export const PASSIVE_INCOME = 0.1;      // ₽/мин: остаточный ручеёк регулярных жертвователей
export const XP_PER_COVER = 0.5;        // отклик за 1% нового покрытия (полный квадрат = 50)
export const XP_MARK = 5;               // отклик за первый разбор находки (📌 или 🗑 — поровну)
export const XP_FIRST = 100;            // порог первого транша — приходит после двух квадратов
export const XP_STEP = 100;             // насколько дорожает каждый следующий
export const DONATION: [number, number] = [150, 350];
export const BIG_DONOR = 0.12;          // шанс, что откликнется крупный жертвователь (×2)
export const XP_TO_MONEY = 0.5;         // курс недобранного отклика в отчёте по делу
/**
 * Затухание интереса: новость живёт двое-трое суток. Первые часы отклик полный, к четвёртым
 * суткам от него остаётся шестая часть. Без этого затянуть поиск снова выгодно: прочёсанные
 * квадраты капали бы деньгами бесконечно (замер: dead-дела приносили больше alive-дел).
 */
export const interestAt = (t: number): number => 0.15 + 0.85 * Math.exp(-t / 1200);
/** Отчёт по делу: сколько принесёт публикация итога. */
export const REPORT_PAYOUT: Record<Outcome, number> = {
  alive: 850, 'alive-late': 400, dead: 150, abandoned: 0,
};
export const CLUE_FEE = 15;             // ₽ за каждую настоящую улику, поднятую в этом деле
