...
<Modal onbackdrop={() => (first ? game.beginCase() : game.closeModal())}>
  ...
  {#if veteran}
    <p class="mut">🏕️ Штаб и команды с прошлых дел прибыли на новое место в полном составе ({g.units.length} {plural(g.units.length, 'отряд', 'отряда', 'отрядов')}). В фонде отряда — <b class="hl">{Math.floor(g.funds)} ₽</b>.</p>
  {/if}
  ...
  <ul>
    ...
    <li><b>Деньги приходят за работу</b>: отряду никто не платит, он живёт на пожертвования. Каждый прочёсанный квадрат и каждая разобранная находка повышают отклик 📣 — на его пороге приходит пожертвование. По итогам дела отряд публикует отчёт: за найденного живым жертвуют щедрее всего. Остаток фонда переходит в следующее дело.</li>
