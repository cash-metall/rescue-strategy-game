<script lang="ts">
  ...
  const won = $derived(game.campaign.stats.won);
  const lost = $derived(game.campaign.stats.lost);
  const veteran = $derived(won + lost > 0);
  const caseNo = $derived(won + lost + 1);
